...
template<class t>
void DCLList<t>::addToHead(t data) {
	DCLLNode<t>* tmp = new DCLLNode<t>(data);
...
template<class t>
void DCLList<t>::addToTail(t data) {
	// if not empty
	if (!isEmpty())
	{
		DCLLNode<t>* tmp = new DCLLNode<t>(data);
...
template<class t>
void DCLList<t>::deleteSLLNode(t nodeData) {
	// if not a empty list
	if (head != nullptr) {
		...
		else
		{
			...
			for (current = head, forward = head->next;
				(forward != tail) && (forward->data != nodeData);
